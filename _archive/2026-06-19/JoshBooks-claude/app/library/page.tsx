...
export default function LibraryPage() {
  ...
  // Manual audiobook merges: { id, title, memberIds } combine selected entries into one playlist.
  ...
  }, [metaOverrides, hiddenIds, links, collectionsApi.collections, collectionsApi.membership, manualGroups]);
  ...
  // Replace merged members with a single synthetic "group:" entry per manual merge.
  const audiobooksAugmented = useMemo(() => {
  ...
  const filteredAudiobooks = useMemo(() => {
    if (!audiobooksAugmented) return [];
    const query = search.trim().toLowerCase();
    const visible = audiobooksAugmented
  ...
  }, [audiobooksAugmented, search, hiddenIds, audioSortField, audioSortDir, metaOverrides, passesCollection]);

  const isGroupId = (id: string) => id.startsWith('group:');
  const groupForId = (id: string) => manualGroups.find((g) => `group:${g.id}` === id);
  ...
  const mergeSelected = () => {
    const ids = [...selectedIds].filter((id) => !isGroupId(id));
    if (ids.length < 2) return;
    const title = window.prompt(`Name this merged playlist (${ids.length} items):`, '');
    if (title === null) return;
    const gid = typeof crypto !== 'undefined' && crypto.randomUUID ? crypto.randomUUID() : `g_${Date.now()}`;
    setManualGroups((prev) => [...prev, { id: gid, title: title.trim() || 'Merged Playlist', memberIds: ids }]);
    setSelectedIds(new Set());
    setSelecting(false);
  };

  const unmerge = (groupEntryId: string) => {
    const g = groupForId(groupEntryId);
    if (g) setManualGroups((prev) => prev.filter((x) => x.id !== g.id));
  };
    ...
    <main className="min-h-screen bg-slate-950 text-slate-100 p-6 sm:p-10">
      <div className="mx-auto max-w-7xl space-y-6">
        <section className="rounded-3xl border border-white/10 bg-slate-900/80 p-6 shadow-xl shadow-black/20">
            ...
            <div className="mt-4 flex flex-col gap-4 lg:flex-row lg:items-center">
              ...
              <div className={`flex flex-wrap items-center gap-3 ${audioSource === 'online' ? 'hidden' : ''}`}>
                ...
                <button
                  ...
                  className={`rounded-2xl border px-3 py-2 text-sm transition ${selecting ? 'border-sky-500 bg-sky-600/20 text-sky-200' : 'border-white/10 bg-slate-950 text-slate-200 hover:bg-slate-800'}`}
                >
          ...
          {tab === 'audiobooks' && audioSource === 'drive' && selecting && (
            <div className="mt-3 flex flex-wrap items-center gap-3 rounded-2xl border border-sky-500/30 bg-sky-600/10 px-4 py-2 text-sm">
              <span className="text-sky-200">{selectedIds.size} selected</span>
              <button
                type="button"
                onClick={mergeSelected}
                disabled={[...selectedIds].filter((id) => !isGroupId(id)).length < 2}
                className="rounded-full bg-sky-600 px-4 py-1.5 text-sm font-semibold text-white transition hover:bg-sky-500 disabled:cursor-not-allowed disabled:bg-slate-700 disabled:text-slate-400"
              >
                Merge into playlist
              </button>
              <button
              ...
              <span className="text-xs text-slate-400">Tick items, then merge. (Open a merged playlist to play all parts; unmerge anytime.)</span>
            </div>
          )}
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-1 text-xs text-slate-500">
                            {isGroupId(book.id)
                              ? `Playlist · ${groupForId(book.id)?.memberIds.length ?? 0} parts`
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-white/5">
                  {filteredAudiobooks.map((book) => {
                    const suggestion = pending[book.id];
                    return (
                      <tr key={book.id} className={`transition hover:bg-slate-800/60 ${suggestion ? 'bg-amber-500/5' : ''} ${selecting && selectedIds.has(book.id) ? 'bg-sky-600/10' : ''}`}>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            {selecting && (
                              <input
                            ...
                            <Link href={`/listen/${book.id}`}>
                              {book.coverUrl ? (
                                ...
                                <div className="flex h-14 w-10 items-center justify-center rounded-md bg-sky-600 text-lg">
                                  {isGroupId(book.id) ? '▶' : '🎧'}
                                </div>
                              )}
                            </Link>
